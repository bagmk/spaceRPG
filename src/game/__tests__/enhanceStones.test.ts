...
describe('P1: 강화석 (enhance stones) two-phase enhancement', () => {
  ...
  it('Lv≥5 success spends BOTH 강화석 and matter and levels up', () => {
    ...
    expect(next.quanta).toBeLessThan(1e9); // stone phase now also costs matter
  ...
  it('Lv≥5 failure drops a level, floored at the stone threshold', () => {
    ...
    // Never below the matter-bought line (= the stone threshold, now Lv3).
    const atFloor = baseState({ quanta: 1e9, enhanceStones: 1000, inventory: [{ entityId: common.id, count: 1, level: ENHANCE_STONE_THRESHOLD }] });
    const floored = gameReducer(atFloor, { type: 'ENHANCE_ENTITY', entityId: common.id, failRoll: 0, destroyRoll: 1 });
    expect(floored.inventory[0].level).toBe(ENHANCE_STONE_THRESHOLD);
  });
  ...
  it('near the cap a failed attempt can destroy a copy (level resets)', () => {
    ...
    const s = baseState({ quanta: 1e9, enhanceStones: 1000, inventory: [{ entityId: common.id, count: 2, level: 9 }] });
    const next = gameReducer(s, { type: 'ENHANCE_ENTITY', entityId: common.id, failRoll: 0, destroyRoll: 0 });
...
describe('P6: enhance fail / phase / destroy math (direct)', () => {
  ...
  it('two-currency phase boundary is exactly Lv3 (#40)', () => {
    expect(isEnhanceStonePhase(ENHANCE_STONE_THRESHOLD - 1)).toBe(false); // last matter level
    expect(isEnhanceStonePhase(ENHANCE_STONE_THRESHOLD)).toBe(true);      // first stone level
  });
