...
function convertEntityModelV14(record: Partial<SaveState>): EntityModelFields {
  ...
  const equippedSlots = isStringArray(record.equippedSlots)
    ...
    : [];
  const unlockedRiftSlotCount = isFiniteNumber(record.unlockedRiftSlotCount)
    ? Math.max(1, Math.min(3, Math.floor(record.unlockedRiftSlotCount)))
    : 1;
...
export function validateV5(
  ...
  return {
    ...
    // v30 강화 보호 charges — WHITELIST: omit and it's silently dropped on load + cloud
    // pull. Clamp ≥ 0; finalizeV17 seeds 0 for pre-v30 saves.
    enhanceProtectCharges: isFiniteNumber((parsed as any).enhanceProtectCharges) ? Math.max(0, Math.floor((parsed as any).enhanceProtectCharges)) : 0,
