...
export type GameAction =
  ...
  // pure/deterministic in tests. failRoll/breakRoll/destroyRoll default to fresh
  // Math.random() in the handler when omitted. destroyRoll decides destroy-vs-유지 on an
  // unprotected fail (< ENHANCE_DESTROY_ON_FAIL → 파괴; else the neutral 유지 — kept at
  // level, cost spent). useProtect = the "보호 사용" toggle (spend a charge to absorb a
